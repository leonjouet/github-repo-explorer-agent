...
async def ingest_repository(
    ...
    try:
        logger.info(f"Starting ingestion for repository: {request.repo_url}")
        ...
        if result.returncode != 0:
            ...
            raise HTTPException(
                status_code=500, detail=f"Failed to ingest repository: {result.stderr}"
            )

        logger.info(f"Successfully ingested repository: {request.repo_url}")

        return {
            "status": "success",
            "message": (
                f"Repository {request.repo_url} has been " f"ingested successfully"
            ),
